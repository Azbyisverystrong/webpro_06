...
app.post("/read", (req, res) => {
  ...
  if( start==0 ) res.json( {messages: bbs, block_name: block_name, good_number: good_number });
  else res.json( {messages: bbs.slice( start ), block_name: block_name, good_number: good_number});
});
...
app.post("/block_post", (req, res) => {
  const block = req.body.block_name;
  console.log([block]);
  block_name.push({block_name: block});
...
app.post("/good_post", (req, res) => {
  ...
  console.log([good]);
  good_number.push({good_number: good});
  res.json({number: good_number.length});
})

app.get("/bbs", (req,res) => {
    console.log("GET /BBS");      // \bbsにgetメソッドでアクセスすると実行．
